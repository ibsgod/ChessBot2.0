...
void Board::place(char pieceChar, int x, int y, bool moved) {
	...
	case 'p':
		...
		if (pieceChar == 'P' && y == 3 && pieces[1][x]->getTeam() == 1 ||
			pieceChar == 'p' && y == 4 && pieces[6][x]->getTeam() == -1) {
			passantPos = { x, y };
		}
		else {
			passantPos = { -1, -1 };
				...
				for (int k = 0; k < 8; k++) {
					for (int l = 0; l < 8; l++) {
						...
						if (castleCheck(j, i, l, k)) {
							...
							place(pieces[k][l]->getTeam() == 1 ? toupper(pieces[k][l]->getPieceChar()) : pieces[k][l]->getPieceChar(), j, i, false);
...
void Board::displayMessages(std::shared_ptr<Xwindow> xw, bool running) {
        // redraws background
